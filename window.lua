...
window.methods = {
    ...
    get_uri_from_hist_string = function(w, d, p)
        local t, ll
        t={}
        ll=0
    ...
    create_pattern = function(w, p)
        local pattern = p
        local self = {}

        function self.get_pattern()
            return pattern
        end

        function self.set_pattern(p)
            pattern = p
        end

        function self.add_pattern(self,p)
            pattern = string.format("%s%s",pattern,p)
        end
        
        return self
    end,

    -- Toggle scrollbar on current view
    toggle_scrollbar = function (w)
        print(w.has_scrollbars)
        if(w.has_scrollbars) then
            print("hide scrollbars")
            w:get_current().show_scrollbars = false
            w.has_scrollbars = false
        else
            print("show scrollbars")
            w:get_current().show_scrollbars = true
            w.has_scrollbars = true
        end
    end,
